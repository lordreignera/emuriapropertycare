...
<div class="row">
    <div class="col-lg-8 mx-auto grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Edit Tool Setting</h4>
                ...
                <form action="{{ route('admin.tool-settings.update', $toolSetting) }}" method="POST" class="forms-sample">
                    @csrf
                    @method('PUT')

                    <div class="form-group">
                        <label for="tool_name">Tool Name <span class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('tool_name') is-invalid @enderror" id="tool_name" name="tool_name" value="{{ old('tool_name', $toolSetting->tool_name) }}" required>
                        @error('tool_name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="form-group">
                        <label for="quantity">Quantity <span class="text-danger">*</span></label>
                        <input type="number" class="form-control @error('quantity') is-invalid @enderror" id="quantity" name="quantity" value="{{ old('quantity', $toolSetting->quantity) }}" min="1" max="999" required onwheel="this.blur()">
                    ...
                    <div class="row">
                        <div class="col-md-6 form-group">
                            ...
                            <select class="form-control @error('ownership_status') is-invalid @enderror" id="ownership_status" name="ownership_status" required>
                                <option value="owned" {{ old('ownership_status', $toolSetting->ownership_status) === 'owned' ? 'selected' : '' }}>Owned</option>
                                <option value="hired" {{ old('ownership_status', $toolSetting->ownership_status) === 'hired' ? 'selected' : '' }}>Hired</option>
                            </select>
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <select class="form-control @error('availability_status') is-invalid @enderror" id="availability_status" name="availability_status" required>
                                <option value="available" {{ old('availability_status', $toolSetting->availability_status) === 'available' ? 'selected' : '' }}>Available</option>
                                <option value="non_available" {{ old('availability_status', $toolSetting->availability_status) === 'non_available' ? 'selected' : '' }}>Non Available</option>
                            </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" min="0" class="form-control @error('sort_order') is-invalid @enderror" id="sort_order" name="sort_order" value="{{ old('sort_order', $toolSetting->sort_order) }}" onwheel="this.blur()">
                        @error('sort_order')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="form-group">
                        <label for="notes">Notes</label>
                        <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="3">{{ old('notes', $toolSetting->notes) }}</textarea>
                        @error('notes')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="form-check">
                        <label class="form-check-label">
                            <input type="checkbox" class="form-check-input" name="is_active" value="1" {{ old('is_active', $toolSetting->is_active) ? 'checked' : '' }}>
                            Active
                        </label>
                    </div>

                    <div class="mt-4">
                        <button type="submit" class="btn btn-primary me-2"><i class="mdi mdi-content-save"></i> Update Tool</button>
