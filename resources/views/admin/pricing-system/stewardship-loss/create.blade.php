<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Create Stewardship Loss Reduction Entry') }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.stewardship-loss.store') }}">
                        @csrf

                        <div class="mb-4">
                            <label for="cpi_band" class="block text-gray-700 text-sm font-bold mb-2">CPI Band *</label>
                            <input type="text" name="cpi_band" id="cpi_band" value="{{ old('cpi_band') }}" placeholder="e.g., CPI-0, CPI-1, CPI-2" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('cpi_band') border-red-500 @enderror" required>
                            @error('cpi_band')
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" step="0.01" min="0" max="1" name="loss_reduction" id="loss_reduction" value="{{ old('loss_reduction') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('loss_reduction') border-red-500 @enderror" required>
