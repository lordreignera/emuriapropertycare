...
<div class="row">
    <div class="col-lg-8 mx-auto grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Edit Mixed-Use Setting</h4>
                
                <form action="{{ route('admin.mixed-use-settings.update', $mixedUseSetting) }}" method="POST" class="forms-sample">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('setting_name') is-invalid @enderror" 
                               id="setting_name" name="setting_name" value="{{ old('setting_name', $mixedUseSetting->setting_name) }}" required>
                        @error('setting_name')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('setting_key') is-invalid @enderror" 
                               id="setting_key" name="setting_key" value="{{ old('setting_key', $mixedUseSetting->setting_key) }}" required>
                        @error('setting_key')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" class="form-control @error('setting_value') is-invalid @enderror" 
                               id="setting_value" name="setting_value" value="{{ old('setting_value', $mixedUseSetting->setting_value) }}" required>
                        @error('setting_value')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" 
                                  id="description" name="description" rows="3">{{ old('description', $mixedUseSetting->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-check">
                        <label class="form-check-label">
                            <input type="checkbox" class="form-check-input" name="is_active" value="1" 
                                   {{ old('is_active', $mixedUseSetting->is_active) ? 'checked' : '' }}>
                            Active
                        </label>
                    </div>
