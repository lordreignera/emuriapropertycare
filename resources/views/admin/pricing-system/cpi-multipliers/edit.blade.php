...
<div class="row">
    <div class="col-lg-8 mx-auto grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Edit CPI Multiplier</h4>
                
                <form action="{{ route('admin.cpi-multipliers.update', $cpiMultiplier) }}" method="POST" class="forms-sample">
                    @csrf
                    @method('PUT')
                        ...
                        <select class="form-control @error('cpi_band_range_id') is-invalid @enderror" 
                            ...
                            @foreach($cpiBands as $band)
                                <option value="{{ $band->id }}" 
                                    {{ old('cpi_band_range_id', $cpiMultiplier->cpi_band_range_id) == $band->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" step="0.01" class="form-control @error('multiplier_value') is-invalid @enderror" 
                               id="multiplier_value" name="multiplier_value" 
                               value="{{ old('multiplier_value', $cpiMultiplier->multiplier_value) }}" required>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" 
                                  id="description" name="description" rows="3">{{ old('description', $cpiMultiplier->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-check">
                        <label class="form-check-label">
                            <input type="checkbox" class="form-check-input" name="is_active" value="1" 
                                   {{ old('is_active', $cpiMultiplier->is_active) ? 'checked' : '' }}>
                            Active
                        </label>
                    </div>
                    ...
                    <div class="mt-4">
                        <button type="submit" class="btn btn-primary me-2">
                            <i class="mdi mdi-content-save"></i> Update Multiplier
                        </button>
