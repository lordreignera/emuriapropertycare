...
<div class="row">
    <div class="col-lg-10 mx-auto grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Edit Pricing Package</h4>
                <p class="card-description">Update package details and pricing for each property type</p>
                ...
                <form action="{{ route('admin.pricing-packages.update', $pricingPackage) }}" method="POST" class="forms-sample">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="col-md-8">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('package_name') is-invalid @enderror" 
                                       id="package_name" name="package_name" value="{{ old('package_name', $pricingPackage->package_name) }}" required>
                                @error('package_name')
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control @error('sort_order') is-invalid @enderror" 
                                       id="sort_order" name="sort_order" value="{{ old('sort_order', $pricingPackage->sort_order) }}">
                                @error('sort_order')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" 
                                  id="description" name="description" rows="3">{{ old('description', $pricingPackage->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control @error('features') is-invalid @enderror" 
                                  id="features" name="features" rows="5">{{ old('features', is_array($pricingPackage->features) ? implode("\n", $pricingPackage->features) : $pricingPackage->features) }}</textarea>
                        @error('features')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="number" 
                                                   ...
                                                   id="price_{{ $type->id }}" 
                                                   name="prices[{{ $type->id }}]" 
                                                   value="{{ old('prices.'.$type->id, $existingPrices[$type->id] ?? '') }}"
                                                   placeholder="0.00">
                                        </div>
                    ...
                    <div class="form-check mt-3">
                        <label class="form-check-label">
                            <input type="checkbox" class="form-check-input" name="is_active" value="1" 
                                   {{ old('is_active', $pricingPackage->is_active) ? 'checked' : '' }}>
                            Active
                        </label>
                    </div>
