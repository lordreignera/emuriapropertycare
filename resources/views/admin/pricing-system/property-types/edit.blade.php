    ...
    <div class="col-lg-8 mx-auto grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{ route('admin.property-types.update', $propertyType) }}" method="POST" class="forms-sample">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('slug') is-invalid @enderror" 
                               id="slug" name="slug" value="{{ old('slug', $propertyType->slug) }}" required>
                        @error('slug')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        ...
                        <div class="form-check">
                            <label class="form-check-label">
                                <input type="checkbox" class="form-check-input" name="uses_residential_pricing" value="1" 
                                       {{ old('uses_residential_pricing', $propertyType->uses_residential_pricing) ? 'checked' : '' }}>
                        ...
                        <div class="form-check">
                            <label class="form-check-label">
                                <input type="checkbox" class="form-check-input" name="uses_commercial_pricing" value="1" 
                                       {{ old('uses_commercial_pricing', $propertyType->uses_commercial_pricing) ? 'checked' : '' }}>
                    ...
                    <div class="mt-4">
                        <button type="submit" class="btn btn-primary me-2">
                            <i class="mdi mdi-content-save"></i> Update Property Type
                        </button>
