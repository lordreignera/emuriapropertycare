                ...
                <form action="{{ route('admin.cpi-domains.factors.update', [$cpiDomain, $factor]) }}" method="POST" class="forms-sample">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('lookup_table') is-invalid @enderror" 
                               id="lookup_table" name="lookup_table" value="{{ old('lookup_table', $factor->lookup_table) }}">
                        @error('lookup_table')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control @error('calculation_rule') is-invalid @enderror" 
                                  id="calculation_rule" name="calculation_rule" rows="3">{{ old('calculation_rule', is_array($factor->calculation_rule) ? json_encode($factor->calculation_rule) : $factor->calculation_rule) }}</textarea>
