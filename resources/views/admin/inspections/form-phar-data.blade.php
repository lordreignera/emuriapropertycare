                                            ...
                                            <tbody>
                                                @foreach($scopedFindings2 as $fi => $finding)
                                                    @php
                                                        ...
                                                        $tradeChecked2 = old("findings.$fi.requires_trade_pricing", array_key_exists('requires_trade_pricing', $finding) ? (int) $finding['requires_trade_pricing'] : (int) $tradeShouldPrice2);
                                                        $fulfillmentType2 = old("findings.$fi.fulfillment_type", $finding['fulfillment_type'] ?? ($trade2['fulfillment_type'] ?? ($tradeShouldPrice2 ? 'trade_partner' : 'etogo_team')));
                                                        $selectedTradeApp2 = old("findings.$fi.trade_application_id", $finding['trade_application_id'] ?? ($trade2['trade_application_id'] ?? ''));
                                                        $tradeQty2 = old("findings.$fi.trade_quantity", $finding['trade_quantity'] ?? ($trade2['quantity'] ?? 1));
                                                        $tradeUnit2 = old("findings.$fi.trade_unit", $finding['trade_unit'] ?? ($trade2['unit'] ?? 'ls'));
                                                    ...
                                                    <tr style="border-left:4px solid {{ $color2 }};{{ $isApprovedScopeLocked && !$isApprovedRow2 ? 'opacity:.62;background:#fff7ed;' : '' }}">
                                                        ...
                                                        <td class="text-end" style="font-size:.8rem;">
                                                            ...
                                                            @endunless
                                                            @if($tradeClient2 > 0)
                                                                <div class="fw-semibold">${{ number_format($tradeClient2, 2) }}</div>
                                                                @if(!empty($trade2['fulfillment_type']))
                                                                    <div class="text-muted">{{ ucwords(str_replace('_', ' ', $trade2['fulfillment_type'])) }}</div>
                                                                @endif
                                                                @if(!empty($trade2['scope_area']))
                                                                    <div class="text-muted">{{ $trade2['scope_area'] }}</div>
                                                                @endif
                                                                @if(!empty($trade2['quantity']) || !empty($trade2['unit']) || !empty($trade2['trade_unit_cost']))
                                                                    <div class="text-muted">{{ number_format((float) ($trade2['quantity'] ?? 1), 2) }} {{ $trade2['unit'] ?? '' }} x ${{ number_format((float) ($trade2['trade_unit_cost'] ?? 0), 2) }}</div>
                                                                @endif
                                                                @if(!empty($trade2['estimated_duration_hours']))
                                                                    <div class="text-muted">{{ number_format((float) $trade2['estimated_duration_hours'], 2) }} expected hrs</div>
                                                                @endif
                                                                <div class="text-muted">cost ${{ number_format($tradeCost2, 2) }}</div>
                                                                <div class="text-success">margin ${{ number_format($tradeMargin2, 2) }}</div>
                                                                <div class="text-muted">{{ $trade2['trade_company_name'] ?? ($trade2['pricing_source'] ?? 'default') }}</div>
                                                            @else
                                                                <span class="text-muted fst-italic">None</span>
                                                            @endif
                                                        </td>
